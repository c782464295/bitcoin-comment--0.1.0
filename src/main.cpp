...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	// 2009年1月3日,财政大臣正站在第二轮救助银行业的边缘
        ...
        txNew.vout[0].nValue       = 50 * COIN;//区块奖励50BTC
        txNew.vout[0].scriptPubKey = CScript() << CBigNum("0x5F1DF16B2B704C8A578D0BBAF74D385CDE12C11EE50455F3C438EF4C3FBCF649B6DE611FEAE06279A60939E028A8D65C10B73071A6F16719274855FEB0FD8A6704") << OP_CHECKSIG;
        CBlock block; // 初始化区块
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;// 前一区块hash=0
        block.hashMerkleRoot = block.BuildMerkleTree();// 计算交易的Merkle值
        block.nVersion = 1;// 版本号
        block.nTime    = 1231006505; //北京时间为2009/1/4 2:15:5
        block.nBits    = 0x1d00ffff; // 记录本区块难度
        block.nNonce   = 2083236893; // 随机数
//其中nNonce 的值设定使得该块的hash是以一串0开头的。
//对于块数据的一点点改变（比如nonce）都会引起block hash的巨大变化。
//由于逆向预测hash值相对应的一组bit值（hash原文）是不可行的，
//试足够多的nonce值且计算每个nonce值相对应的block hash之后可以找到一个满足有指定数量 0 bits (0比特位) 的hash值。
	...
	// 文件名为blk0001.dat，后续区块名字递增，如blk0002.dat …
        ...
        if (!block.AddToBlockIndex(nFile, nBlockPos)) // 其功能包含重复性区块检查、最好分支处理、新最好分支处理等等…
