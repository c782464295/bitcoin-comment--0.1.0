...
public:
    // socket
    uint64 nServices;
    SOCKET hSocket; // socket实例
    CDataStream vSend; // 发送缓存区
    CDataStream vRecv; // 接收缓冲区
    CCriticalSection cs_vSend; // 发送互斥锁
    CCriticalSection cs_vRecv; // 接受互斥锁
    ...
    bool fDisconnect; // 端口链接的标识
protected:
    int nRefCount; // 使用计数器
public:
    int64 nReleaseTime; // 节点释放的时间
    map<uint256, CRequestTracker> mapRequests;
    CCriticalSection cs_mapRequests; // 互斥锁
